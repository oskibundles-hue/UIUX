...
**Full-bleed 4x5, 24 Sept.** `build_edit.py --canvas 4x5 --crop-aim 0.25`
renders the ad AT 1080x1350 instead of reframing a finished 9x16 master, so
the picture goes edge to edge with no blurred rails. Two things made it
possible and neither was a rewrite:

- The layout functions have always taken `canvas` and read `B.CANVASES[canvas]`
  - no geometry was hard-coded to 1920 height. The full-frame overlays were
  simply only ever GENERATED for 9x16 and 16x9. `build_overlays.py` now uses
  `FULL_FRAME_CANVASES` and emits title cards, lower thirds and CTA captions
  for 4x5 and 1x1 as well: 136 new files, and the existing 9x16 and 16x9
  assets re-generated byte-identical, so the approved series is untouched.
- `--canvas` picks the target and `source_fit()` scales the source to COVER it
  and crops the excess. `--crop-aim` says where that crop window sits.

**The crop costs 30% of the vertical field of view** - 570px of the 1920. That
is the honest trade against the reframe, which keeps the whole field and spends
30% of the canvas on blurred filler instead.

**Aim 0.25, chosen against the measurement rather than with it.** Row-wise
gradient energy over 68 sampled frames ranked aim 0.00 highest (80.0% of the
detail retained, against 77.6% at 0.25 and 76.0% at centre). Looking at the
crops, 0.00 is wrong at every beat: it keeps dead warehouse floor at the top
and cuts the oil bottles, the rear bumper and plate, and the hand holding the
bottle. The metric was measuring texture, and the concrete floor is textured
and worthless. 0.25 keeps the subject whole in all three beats. Measure first,
but look at the frames before believing the number.

Verified after rendering: lowest graphic ink sits 147-162px off the bottom
(10.9-12.0% of height), clear of the 9% keep-out the generator uses for
non-9x16 canvases, and the hook sets on one line with 235px margins either
side.

**The renderer is not bit-reproducible, and that is why the pair is muxed
rather than rendered twice.** Two runs of the same command on the same code
produce different pictures from frame 22 on. It is not the cue sheet and not
the overlays - the plan is identical and the motion frames hash the same
across processes. It is x264: encoding the same three seconds twice with
`-preset medium -b:v 9M` gives different output, and the same encode with
`-threads 1` is exact. So picture parity between the sound-designed and
picture-only versions can never come from rendering twice. Render ONCE, then
copy the video stream and swap the audio:

```bash
ffmpeg -i <sfx render> -i <source> -map 0:v -c:v copy -map 1:a \
       -af apad -t <duration read off the render> -c:a aac -b:a 320k out.mp4
```

`-shortest` is still wrong here for the reason recorded below. Take the
duration off the rendered file.
...
| **Video — oil service · white 911, real oil-change footage** | **Approved 23 Sept** — the starter for the new series. Shipped 9x16 + full-bleed 4x5, both ratios in sound-designed and picture-only |

...
def build_swatches():
    ...
    for name, hex_code, _ in B.PALETTE:
        ...
        R.save(im, out / f"sce-{name}-{hex_code.lstrip('#')}.png")
    ...
    for width in (1080, 1920, 2160):
        for label, height in (("thin", round(width * 0.011)),
            ...
            R.save(R.accent_stripe(width, height),
                   out / f"sce-accent-stripe_{width}w-{label}.png")
            solid = Image.new("RGBA", (width, height), B.rgb(B.RED) + (255,))
            R.save(solid, out / f"sce-gold-bar_{width}w-{label}.png")
            n += 2
    return n
    ...
    for canvas, (fw, fh) in B.CANVASES.items():
        ...
        for lockup, share in (("primary-horizontal", 0.30), ("icon-mark-only", 0.09)):
            for tone in ("white", "black"):
                mark = R.logo(f"sce-{lockup}--{tone}", width=round(fw * share))
...
def build_lower_thirds():
    ...
    for canvas in ("9x16", "16x9"):
        ...
        for slug, label, _prose, category in B.PARTNERS:
            R.save(lower_third(canvas, label, category, "LOCATION"),
                   out / f"lt_{canvas}_partner_{slug}.png")
            n += 1
        R.save(lower_third(canvas, "BOOK YOUR RIDE", B.WEBSITE, "TEXT TO BOOK"),
               out / f"lt_{canvas}_cta_book-your-ride.png")
...
TITLES = [
    ...
    ("4-hours-or-24", "4 HOURS", "OR 24"),
    ("pick-your-car", "PICK YOUR", "CAR"),
    ("the-fleet", "THE", "FLEET"),
    ("book-the-weekend", "BOOK THE", "WEEKEND"),
    ("ride-of-a-lifetime", "A RIDE OF", "A LIFETIME"),
    ("las-vegas", "LAS VEGAS", "EXOTICS"),
    ("scottsdale", "SCOTTSDALE", "EXOTICS"),
    ("fall-rally", "FALL", "RALLY"),
    ("one-of-one", "TRULY", "1 OF 1"),
]
...
def end_card_block(canvas, tone):
    ...
    lockup = "sce-stacked" if canvas in ("9x16", "4x5", "1x1") else "sce-primary-horizontal"
    mark = R.logo(f"{lockup}--{'white' if tone == 'dark' else 'black'}",
                  width=round(w * (0.62 if lockup == "sce-stacked" else 0.84)))

...
|---|---|---|---|
| 0.0 → 0.2 | 0 | 1.1.1 | Crop marks draw from each vertex outward: arm = 28·swift(clamp((t + 1/60)/0.2)); frame 0 already shows ~45%. All labels are fully set on frame 0 (the hook needs no fade). |
...
| t (s) | Frame | Pos | Action |
|---|---|---|---|
| 0.0 → 0.234375 | 0 | 1.1.1 | Frame 0 is mid-motion. "LIGHT" at wght 100, font-stretch 100%. Letters rise out of the baseline mask (translateY 110%→0, swift, 0.28 s, 1-frame stagger L→R); the tween clock starts at t = −0.06, so frame 0 shows ~35% of every hairline letter. Construction lines shoot outward from x=960 to both edges (outCubic, 0→0.234). Readout types on 0.03→0.15. |
...
| t (s) | Frame | Pos | Action |
|---|---|---|---|
| 1.875 → 1.9921875 | 113 | 2.1.1 | ANTICIPATION (one 16th): the dot squashes to scaleX 1.3 / scaleY 0.77 anchored at its bottom (y=700) and leans left (skewX 8°). Label "ANTICIPATION" right-aligned at (1700, baseline 596), leader to the top of the dot. |
...
- FINAL LOCKUP (everything below is static from 14.0625):
  ...
  - Monogram "C." (original mark): box x 200→272, y 944→1016 (the old HUD chapter slot, aligned to the name's left edge). A Paper ring centred (236,980), centreline radius 28, stroke 12, butt caps, with a 60° opening centred at 3 o'clock (the arc runs from 30° to 330°); plus a Signal Ø16 dot that lands in the opening at (264,980).
...
| 13.125 → 13.59375 | 788 | 8.1.1 | THE BOW: the expansion flings the dot. x = 1542 + 222τ, y(τ) = 638 − 68τ − 4·403.3·τ(1−τ), τ = (t − 13.125)/0.46875 (apex ≈200 at τ 0.52), always clear of the expanding E. Diameter 64→60. Smear frames on the first 2 frames (stretch 2.2 then 1.4 along velocity), then a normal stretch in flight. |
...
| 7 | 11.25 | **RANGE II → SQUEEZE** | 1.0 | Fm9 . Eb . | 11.25 a glitch stutter (the previous 8th re-triggered in 32nds, bitcrushed); 11.484375 a stab + snare + 3 shwips; 16th stutters rising F→Ab→C on the row pops; a buffer-repeat on 12.0703; 12.1875 a big Fm(add9) stab + clap + plip (the period lands). 12.65625 (beat 4): drums out, leaving a riser, a 32nd snare roll, a sub inhale and the reverse swell of the final hit. 13.0078125→13.125: total silence except the swell's peak (the gap). | stutter '8d.. .... b... xxxx' (beat 1 retrigger + crush; beat 4 muted); snare '.... .... .... 789X' then mute the last 16th; level ramp to −inf on the last 16th except the reverse FX. |
...
- Key: F minor (matches audio/synth.py) instead of the pitches' A minor; note names in the sound notes are written in F minor.

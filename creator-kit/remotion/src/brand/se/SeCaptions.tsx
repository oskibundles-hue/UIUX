 ...
 * words still to come are white 36%. Orange is not used here.
 *
...
export const captionPages = (words: Word[], breaks: number[], after: number): CapPage[] => {
  const phrases = carrySentenceStarts(toPhrases(words.filter((w) => w.start >= after), breaks), breaks);
  const groups: Word[][][] = [];
  ...
  const pages: CapPage[] = groups.map((group) => {
    ...
    return { lines: layoutLines(flat, inner, group[0].length), start: flat[0].start, lastEnd: flat[flat.length - 1].end, tailEnd: 0, prevJoin: false, nextJoin: false };
  });
  pages.forEach((p, i) => {
        ...
        return (
          <div key={li} style={{ ...font(withSize(WORD, size)), lineHeight: CAP.lineHeight, whiteSpace: "nowrap" }}>
            {line.map((w, wi) => {
              ...
              const color = isChip ? C.chipInk : j <= now ? C.ink : C.ink36;
              return (
                <React.Fragment key={wi}>
                  {wi > 0 ? " " : null}
                  <span style={{ position: "relative", padding: `0 ${CAP.chipPadX}px`, margin: `0 -${CAP.chipOverlap}px` }}>
                    {isChip ? <span style={{ position: "absolute", left: 0, right: 0, top: "0.07em", bottom: "-0.03em", background: C.chip, borderRadius: CAP.chipRadius,

...
/**
 * Booking CTA (new, mid-roll): kicker "NOW BOOKING" in white 80% behind a red live lamp blinking at 2 Hz (red stays on the
 * lamp, the live signal: red kicker type measured ~2:1 on bright plates), the action "BOOK YOUR BUILD" at 250 px, a rule with
 * its red first third, "DM US YOUR MODEL" 110 px and the handle 76 px grey; haze with the graph grid (alpha: props `haze`
 * from tools/fd_element_haze.py, else the panel default). Default slot = the lower-third slot, bottom edge at y 2940 (or
 * `y`, 0-1). Captions and the chapter bar yield for 8 f around it (FdMotion). Motion: haze 8 f, kicker types 10 f, action
 * clips in 12 f, rule 14 f, how and handle type; out 8 f fade.
 */
export const FdCta: React.FC<{ c: BrandCta; copy: FdCopy }> = ({ c, copy }) => {
  ...
  const kick = up(copy.kicker), act = up(copy.cta), how = up(copy.how), handle = up(copy.handle);
  ...
  const handleSt = fitSize([handle], HANDLE, maxW, 48);
  const w = Math.ceil(Math.max(30 + CT.lampGap + tw(kick, kickSt), tw(act, actSt), tw(how, howSt), tw(handle, handleSt)));
  const h = T.ctaKicker + CT.actGap + actSt.size * 0.82 + CT.ruleGap + CT.ruleH + CT.howGap + howSt.size + CT.handleGap + handleSt.size;
  ...
  const handleK = ramp(f0, 22, 22 + Math.ceil([...handle].length / 2), linear);
  ...
  return (
    <div style={{ position: "absolute", left: 0, top: 0, width: "100%", height: "100%", opacity: 1 - out }}>
      ...
      <div style={{ position: "absolute", left: CT.left, top, width: w, whiteSpace: "nowrap", textShadow: SHADOW }}>
        ...
        <div style={{ ...font(handleSt), lineHeight: 1, marginTop: CT.handleGap, color: C.sub }}><TypeOn text={handle} k={handleK} /></div>
      </div>
    </div>
  );
};
